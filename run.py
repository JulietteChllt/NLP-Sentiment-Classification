...
# Data Organization
pos_path = "Resources/train_pos_full.txt"
neg_path = "Resources/train_neg_full.txt"
...
train_dataset = TweeterDataset(train_df, tokenizer_name)
valid_dataset = TweeterDataset(test_df, tokenizer_name)
...
class SentimentClassifier(nn.Module):
    ...
    def __init__(self, model_name):
        ...
        if (model_name == 'XtremeDistil-l12-h384' or model_name == 'XtremeDistil-l6-h384'):
            output_size = 384
...
for i in range(len(evaluation_df)):
    ...
    with torch.no_grad():
        model.to(device)
        preds = model(encodings['input_ids'].to(
            device), encodings['attention_mask'].to(device)).to('cpu')
...
df_to_submit.to_csv(
    'Resources/'+model_name, index=False)

# Saving trained model
torch.save(model, 'Resources/'+model_name+'.pth')
